...
const CommentSection = ({ resourceId, currentUser }) => {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const response = await axios.get(`https://localhost:7196/api/Comment/resource/${resourceId}`);
        setComments(response.data);
        console.log('CommentSection - fetchComments - success:', response.data);
      } catch (error) {
        console.error(error);
  ...
  return (
    ...
    {comments.length > 0 ? (
      ...
      <p>Aucun commentaire sur  cette ressource</p>
    )}
